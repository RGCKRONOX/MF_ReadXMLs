    ...
    public class SQLSRV
    {
        ...
        public SQLSRV()
        {
            try
            {
                string gCadenaConexion =
                            $@"Data Source={Globales.serviceConfig.dbInstance};
                                Initial Catalog={Globales.serviceConfig.dbDatabase};
                                Integrated Security=False;
                                MultipleActiveResultSets=True;
                                User Id={Globales.serviceConfig.dbUser};
                                Password={Globales.serviceConfig.dbPass};";
        ...
        public int Conecta()
        {
            ...
            catch (Exception ex)
            {
                ...
                Globales.logs.add("Error Conecta: " + gSqlConnection.DataSource + ex.StackTrace);
                return 0;
            }
        }
        ...
        public SqlDataReader EjecutarLecturaDeDatos(string p_sentencia)
        {
            ...
            {
                Console.WriteLine(ex.Message);
                Globales.logs.add("Error - SQL Lectura de datos : " + p_sentencia + " - " + ex.Message);
                return null;
            }
        }
        public int EjecutaSimpleQuery(string query)
        {
            ...
            {
                Console.WriteLine(ex.Message);
                Globales.logs.add("Error - SQL Lectura de datos : " + query + " - " + ex.Message);
                return 0;
            }
        }
